...
### v1.3.2 - 功能修复与体验优化 (2025-05-30)

#### 🎯 重大更新
- 新用户首次启动不再自动生成任何默认提示词、API配置、模型等数据，数据完全由用户自主添加。
- API配置页面弹窗由Modal改为宽屏Drawer，表单空间更大，模型选择体验优化。
- 测试配置统计卡片美化，增加今日测试次数、累计测试次数等统计数据，排版更合理。
- 语言切换按钮由单按钮改为下拉菜单，支持多语言扩展。
- API Key输入框默认星号隐藏，提升安全性。
- 默认用户头像本地化，加载更快。
- Electron打包应用自定义图标，支持macOS和Windows平台。
- 根目录清理所有无用测试/调试/临时文件，文档结构规范化。

#### ✨ 新增/优化功能
- 支持统计今日测试次数、累计测试次数，统计数据自动从历史记录中获取。
- Drawer表单支持横向排布，提升模型选择和配置体验。
- 统计卡片顶部为"测试统计"+小图标，横向展示多项统计数据。
- 语言切换下拉菜单支持未来多语言扩展。
- 用户头像优先本地加载，提升体验。
- 应用打包图标自动集成icon.iconset多分辨率图标。

#### 🐛 问题修复
- 修复打包后新用户有历史测试数据和API配置的问题。
- 修复API Key明文显示的安全隐患。
- 修复部分页面国际化遗漏，所有统计相关字段均已国际化。
- 修复根目录存在大量无用测试/调试/临时文件的问题。
- 修复打包后白屏问题，静态资源路径和图标引用全部规范化。
- 修复GitHub推送大文件失败，彻底清理历史大文件并规范.gitignore。

#### 📦 技术变更
- `src/components/TestingPanel.tsx` 统计卡片与数据逻辑优化
- `src/components/ApiConfigManagement.tsx` Modal改Drawer及表单体验优化
- `src/components/UserProfile.tsx` 头像本地化
- `src/App.tsx` 语言切换下拉菜单
- `package.json`、`electron/main.ts` 图标配置与打包优化
- 根目录及.gitignore清理与规范

#### ⚠️ 已知问题与限制
- 目前仅支持本地数据存储，暂不支持云端同步
- 测试历史容量有限，建议定期导出重要数据
